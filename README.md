# 📊 Análise Salarial TI

Este projeto é uma **evolução do ETL_SALARIOS_TI**.  
Aqui utilizamos os dados já tratados no ETL para realizar **análises exploratórias e visualizações interativas** em ambiente **Databricks**, com **Pandas** e **Plotly**.

---

## 🚀 Objetivo
- Explorar os dados salariais de profissionais de TI.
- Gerar gráficos comparativos por país, senioridade, modalidade de trabalho e tamanho de empresa.
- Obter insights sobre distribuição salarial.

---

## 🛠️ Tecnologias utilizadas
- **Databricks** (execução do notebook)
- **Python 3**
- **Pandas** (manipulação de dados)
- **Plotly Express** (visualizações interativas)

---
## 📌 Observação
Este projeto foi desenvolvido e executado na plataforma **Databricks**.  
Ao exportar o notebook para formato `.ipynb`, as saídas gráficas não são incluídas automaticamente.  
Por isso, os gráficos não aparecem diretamente no notebook exportado.  

No entanto, todo o código pode ser executado em ambientes como **VS Code** ou **Jupyter Notebook**, onde as visualizações seriam plotadas normalmente.  
Para fins de apresentação, os gráficos já estão disponíveis na pasta `assets` e exibidos neste README.


---

## 📊 Principais análises
## 1. Qual o nível de experiência mais comum na base? 
![Gráfico experiencia](asset/01_grafico_experiencia.png) 
## 2. Top 10 países com maior média salarial.  
![Gráfico media](asset/02_grafico_media_paises.png)
## 3. Existe relação entre trabalho remoto e salário? 
![Gráfico relacao](asset/03_grafico_relacao_remoto.png) 
## 4. Qual o cargo mais bem pago por senioridade?  
![Gráfico senioridade](asset/04_grafico_senioridade.png)
## 5. Como varia o salário médio por tamanho da empresa? 
![Gráfico tamanho](asset/05_grafico_tamanho_empresa.png) 
## 6. Qual o crescimento salarial ao longo dos anos?
![Gráfico crescimento](asset/06_grafico_crescimento.png)
## 7. Há diferença entre salário bruto e salary_usd por região?
![Gráfico bruto vs USD](asset/07_grafico_bruto_usd.png)
## 8. Qual o percentual de profissionais remotos vs presenciais?
![Gráfico percentual](asset/08_grafico_modalidade.png)
## 9. Quais países concentram mais profissionais de TI?
![Gráfico concentracao](asset/09_grafico_concentracao_paises.png)
## 10. Qual o impacto da senioridade no trabalho remoto?
![Gráfico senioridade](asset/10_grafico_senioridade_remoto.png)

# Fim
